...
//
func (m *Coordinator) GetReduceCount(args *GetReduceCountArgs, reply *GetReduceCountReply) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// RequestTask RPC handler.
//
func (m *Coordinator) ReportTaskDone(args *ReportTaskArgs, reply *ReportTaskReply) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *Coordinator) selectTask(taskList []Task, workerId int) *Task {
	var task *Task
